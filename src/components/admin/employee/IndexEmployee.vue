<template>
    ...
    <div class="flex">
        ...
        <div class="bg-sky-50 grow h-screen px-10 py-6">
            ...
            <div class="flex items-center align-center my-4 bg-white px-4 py-3 rounded-md">
                <p class="text-2xl font-medium">Employee</p>
                <router-link :to="{name: 'createEmployee'}" class="ml-auto px-4 py-2 bg-sky-500 rounded-md text-white font-medium">Create new</router-link>
            </div>
            ...
            <table class="w-full text-left bg-white rounded-lg">
                ...
                <tbody v-for="employee in employees" :key="employee.id">
                        <tr class="text-gray-600 text-sm">
                            ...
                            <td class="px-4 py-3">
                                <p v-if="employee.role === 'admin'" class='text-red-500 rounded-md font-medium'>{{ employee.role }}</p>
                                <p v-else class='text-green-500 rounded-md font-medium'>{{ employee.role }}</p>
                            </td>   
                            <td class="px-4 py-3">
                                <p v-if="employee.status === 'active'" class='bg-sky-500 text-white rounded-md inline-block text-center font-medium py-0 px-2'>{{ employee.status }}</p>
                                <p v-else class='bg-zinc-400 text-white rounded-md inline-block text-center font-medium py-0 px-2 text-sm'>{{ employee.status }}</p>
                            </td>
                            <td class="lg:pl-4 py-3 text-zinc-500 hover:underline flex gap-3 items-center">
                                <router-link :to="{name: 'detailEmployee', params: {id: employee.id}}">
